...
vocal_end_time_fp = './misc/mauch_vocal_end_time.json'
segmented_dataset_dir = './segmented/mauch'
metadata_fp = jpath(segmented_dataset_dir, 'metadata_unnormalized.json')
utter_audio_dir = jpath(segmented_dataset_dir, 'data_seg') # save segmentation of original wav file
utter_audio_resample_dir = jpath(segmented_dataset_dir, 'data')  # save 16 kHz wav
normalized_meta_fp = jpath(segmented_dataset_dir, 'metadata.json')

def procedures():
    source_separate_for_mauch()
...
def get_utter_annotation():
    ...
    annotation_folder = jpath(dataset_dir, 'wordonsets')
    lyrics_folder = jpath(dataset_dir, 'lyrics')
    lyric_files = ls(lyrics_folder)
    audio_names = [t[:-4] for t in lyric_files]
    pbar = tqdm(audio_names)
    for audio_name in pbar:
        pbar.set_description(audio_name)
        
        lyric_path = jpath(lyrics_folder, '{}.txt'.format(audio_name))
        annotation_path = jpath(annotation_folder, '{}.wordonset.tsv'.format(audio_name))
...
def read_annotation_file(path):
    ...
    try:
        for line in text:
            onset_time, lyric_word = line.split('\t')
...
def segment_by_annotation():
    ...
    '''
    create_dirs_if_not_exist(segmented_dataset_dir)
