...
def mix_datasets_txt():
    # mix the text from different datasets
    root = 'mix_data'
    libri_csv = os.path.join(root, 'librispeech_train.csv')
    dsing_csv = os.path.join(root, 'dsing_train.csv')
    save_txt = os.path.join(root, 'train.txt')

    dict_data1 = load_data_csv(libri_csv)
    dict_data2 = load_data_csv(dsing_csv)
    with open(save_txt, 'w') as f:
        for key in dict_data1.keys():
            wrd = dict_data1[key]['wrd']
            f.write(wrd)
            f.write('\n')
        for key in dict_data2.keys():
            wrd = dict_data2[key]['wrd']
            f.write(wrd)
            f.write('\n')


def mix_datasets_csv():
    # mix the text from different datasets
    root = 'mix_data'
    libri_csv = os.path.join(root, 'librispeech_train.csv')
    dsing_csv = os.path.join(root, 'dsing_train.csv')
    save_path = os.path.join(root, 'train.csv')

    dict_data1 = load_data_csv(libri_csv)
    dict_data2 = load_data_csv(dsing_csv)
    csv_lines = [["ID", "length", "wrd"]]
    for key in dict_data1.keys():
        id = key
        wrd = dict_data1[key]['wrd']
        length = len(list(wrd))
        csv_line = [id, str(length), wrd]
...
def transform_csv():
    ...
    splits = ['dev', 'test']
    for split in splits:
        dsing_csv = os.path.join(root, 'dsing_' + split + '.csv')
        dict_data = load_data_csv(dsing_csv)
        csv_lines = [["ID", "length", "wrd"]]
        save_path = os.path.join(root, split + '.csv')
        for key in dict_data.keys():
            id = key
            wrd = dict_data[key]['wrd']
            length = len(list(wrd))
            csv_line = [id, str(length), wrd]
            csv_lines.append(csv_line)
